...
export const PersonalDictionaryModal: React.FC<PersonalDictionaryModalProps> = ({
  ...
  const allDictionaryEntries: StudentDictionaryEntry[] = useMemo(() => {
    ...
    Object.entries(COMMON_ROUTINE_DICTIONARY).forEach(([key, val]) => {
      map.set(key.toLowerCase(), {
        id: `dict-${key}`,
        word: val.word,
        definitionEn: val.definitionEn,
        partOfSpeech: val.partOfSpeech,
        exampleSentenceEn: val.exampleSentenceEn,
    ...
    wordsFromRoutines.forEach((item) => {
      ...
      if (existing) {
        ...
        const generated = getDictionaryDefinition(cleanWord, item.sourceActivityName);
        map.set(lower, {
          id: `routine-${lower}`,
          word: cleanWord,
          definitionEn: generated.definitionEn,
          partOfSpeech: generated.partOfSpeech,
          exampleSentenceEn: generated.exampleSentenceEn,
          translationPt: generated.translationPt,
  ...
  }, [wordsFromRoutines, customSavedEntries]);
  ...
  const handleLookupWordWithAi = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.definitionEn) setNewDefinition(data.definitionEn);
        if (data.exampleSentenceEn) setNewExample(data.exampleSentenceEn);
        if (data.translationPt) setNewTranslation(data.translationPt);
      }
    } catch (e) {
      console.warn('AI lookup error:', e);
      const def = getDictionaryDefinition(newWord.trim());
      setNewDefinition(def.definitionEn);
      setNewExample(def.exampleSentenceEn || '');
      setNewTranslation(def.translationPt || '');
    } finally {
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-extrabold text-[#000035] flex items-center gap-1.5">
                ...
                <span>{isEn ? 'Add Custom Word to Your Dictionary' : 'Adicionar Nova Palavra ao seu Dicionário'}</span>
              </span>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
              <div className="sm:col-span-4">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button
                    type="button"
                    onClick={handleLookupWordWithAi}
                    disabled={isLookingUpAi || !newWord.trim()}
                    className="px-2.5 py-1.5 bg-[#1C4C96] hover:bg-[#062863] text-white rounded-xl text-[10px] font-bold disabled:opacity-50 transition cursor-pointer shrink-0"
                    title={isEn ? 'Auto-generate definition with AI' : 'Gerar definição com IA'}
                  >
                    {isLookingUpAi ? '...' : <Sparkles className="w-3.5 h-3.5" />}
                  </button>
                </div>
              </div>
              ...
              <div className="sm:col-span-8">
                <label className="block text-[11px] font-bold text-slate-700 mb-1">
                  {isEn ? 'English Definition' : 'Definição (em Inglês)'} *
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="sm:col-span-8">
                <label className="block text-[11px] font-bold text-slate-700 mb-1">
                  {isEn ? 'Example Sentence' : 'Frase de Exemplo em Inglês'}
                </label>
                <input
                  type="text"
                  value={newExample}
                  onChange={(e) => setNewExample(e.target.value)}
                  placeholder="e.g. We streamlined our morning meeting routine."
                  className="w-full p-2 bg-white border border-slate-300 rounded-xl text-xs text-[#000035]"
                />
              </div>
